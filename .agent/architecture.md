...
本文保留项目架构决策的历史记录。当前代码、接口和功能进度以
[docs/14-当前实现同步说明.md](../docs/14-当前实现同步说明.md) 为准。

- 当前运行链路为 MySQL/Redis + Spring Boot + Flask 推理服务 + React 前端。
- LSTM 是后续重训练和主推理模型；Prophet 仅保留后端兼容能力和历史版本兼容。
- `GET /model/versions` 只查询数据库，`POST /model/versions/sync` 才同步本地模型产物。
- 告警研判的 GET 接口只读缓存，显式 POST 才重新调用 LLM；红色告警只生成待确认工单草稿。
- Agent 回复支持 SSE Markdown、图表持久化和数据来源、模型版本、模拟数据标识等可信度信息。
- 实时负荷和恢复段目前仍包含演示模拟数据，不代表真实 SCADA 数据。
...
| 有备选方案 | Flask 主力 / DJL 备选；LSTM 主力 / Prophet 历史兼容 |
...
## ADR-016：Agent Markdown 与图表持久化

**决策日期**：2026-07-19
**状态**：已确认

**背景**：Agent 回复 Markdown 和图表需要刷新后才稳定渲染，且刷新后图表会丢失。

**决策**：SSE `text` 事件一次性发送完整 Markdown；assistant 图表配置保存到 `conversation.chart_option`，历史会话接口返回 `chartOption`。

**影响**：前端打开历史会话时可同时恢复文本和图表，避免图表只存在于当前流式会话内存中。

---

## ADR-017：恢复段到实时段使用前端视觉 bridge

**决策日期**：2026-07-19
**状态**：已确认

**背景**：恢复段与实时曲线存在可视断点，直接补一个整点实时值会造成长直线并污染数据语义。

**决策**：在前端生成 `bridgeData`，使用 15 分钟 smoothstep 插值连接恢复段末点和首个真实实时点。

**影响**：视觉上平滑连续，但不写库、不导出、不影响整点归档和统计口径。

---

## ADR-018：红色告警只生成待确认工单草稿

**决策日期**：2026-07-19
**状态**：已确认

**背景**：红色告警风险高，但自动提交正式工单会绕过调度员确认权，可能造成误派发。

**决策**：红色告警 `shouldCreateTicket=true`、`autoCreateTicket=false`。前端弹出待确认工单草稿，调度员点击“提交工单”后才创建正式 `alert_ticket`。

**影响**：兼顾强提醒和人工确认，正式工单状态不与前端临时草稿混淆。

---

## ADR-019：告警触发模板与工单详情 LLM 研判分层

**决策日期**：2026-07-19
**状态**：已确认

**背景**：用户期望工单详情里的“AI 智能研判”真实调用 LLM，但告警触发链路仍需低延迟。

**决策**：`AlertTemplate` 继续负责告警事件文案；`AlertJudgementService` 在详情页/重新研判时调用 LLM，失败时降级规则研判。

**影响**：告警触发不依赖外部 LLM；详情研判可提供更自然的调度/运维建议。
