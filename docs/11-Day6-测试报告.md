...
| Flask | 3.x, port 5000 |
| Python | 3.9+ (`.venv`) |
| 推理框架 | PyTorch + TorchScript (LSTM), Prophet (fallback) |
| 模型文件 | `models/lstm_scripted.pt`, `models/lstm_meta.pt`, `models/prophet_model.pkl` |
...
|:-----|:-----|
| **用例 ID** | TC-6.1.1 |
| **测试项** | Flask 启动 → 加载 TorchScript LSTM + 回退 Prophet |
| **优先级** | P0 |
...
| **预期结果** | 日志：`Prophet model loaded: models/prophet_model.pkl` <br> `Inference ready (model=prophet)` |
| **实际结果** | |
| **状态** | ✅ 通过 |
