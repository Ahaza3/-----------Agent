...
| 实时推送 | WebSocket 告警推送 | 1 |
| Agent 对话 | 用户查询 → Agent → LLM → 图表 | 1 |
...
|:-----|:-----|
| **用例 ID** | E2E-002 |
| **场景名** | WebSocket 实时告警：阈值触发 → 推送 → 大屏闪烁 |
| **优先级** | P1 |
| **涉及组件** | ThresholdDetector → WebSocket STOMP → React |
| **测试步骤** | 1. 设置阈值告警规则 <br> 2. 注入超过阈值的模拟数据 <br> 3. 观察前端告警卡片 |
| **预期结果** | 1. 负荷超限后 ≤5s 收到告警 <br> 2. 前端红色闪烁动画 <br> 3. 告警卡片显示时间/级别/当前值/阈值 |
| **状态** | ✅ 通过 |

### E2E-003：Agent 自然语言查询

| 字段 | 内容 |
|:-----|:-----|
| **用例 ID** | E2E-003 |
| **场景名** | Agent 对话：用户输入 NL → SSE 流式 → 图表响应 |
| **优先级** | P1 |
| **涉及组件** | React Chat UI → Spring SSE → AgentCore → LLM API → ToolRegistry |
| **测试步骤** | 1. 打开 Agent 对话页面 <br> 2. 输入 "今天下午的负荷峰值是多少？" <br> 3. 观察 SSE 流式输出 |
| **预期结果** | 1. SSE 事件流：`thinking` → `text` → `chart` → `done` <br> 2. 最终显示自然语言回答 + ECharts 图表 <br> 3. 响应时间 < 5s |
| **状态** | ✅ 通过 |
...
| **本文档** | Sprint 1 全量测试用例框架（D4 交付物） | 39 |

---

> **Sprint 1 测试总计**：**39 条用例**（单元 19 + 集成 17 + E2E 3）  
