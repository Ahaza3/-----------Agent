...
### 8.4 当前实现基准（2026-07-20）

- 模型版本列表是纯查询，模型文件同步由独立的 `/model/versions/sync` 负责。
- 告警研判 GET 只读取缓存，无缓存时不触发 LLM；重新研判使用显式 POST。
- 红色告警统一称为“待确认工单草稿”，正式工单必须由调度员确认提交。
- 产品已决定后续使用 LSTM，当前 Prophet 页面和后端兼容入口待下一步移除，历史版本暂保留。
- 完整同步清单见 [当前实现同步说明](./14-当前实现同步说明.md)。
