...
| **有备选方案** | 每个技术风险点都有 fallback | DJL 备选 Flask 微服务；LSTM 备选 Prophet 基线 |
...
告警模块（分阶段交付）
├── P0（Day 7-8）: 阈值检测
│   └── ThresholdDetector      # 当前值 > 阈值 → 告警
│
├── P1（Day 9-11，有余力）: 趋势检测
│   └── TrendDetector           # 预测值即将超限 → 预警（加分项）
│
└── 告警引擎（核心，P0 必须）
    ├── LevelClassifier         # 分级: 红(>110%) / 橙(>100%) / 黄(>90%)
    ├── AlertTemplate           # 固定模板生成告警文案（不用 LLM！）
...
| 表 | 索引 | 用途 |
|----|------|------|
| load_data | `idx_time` (UNIQUE) | 按时间查询负荷（最频繁） |
| load_data | `idx_hour` | 按小时聚合 |
| prediction_result | `idx_predict_time` | 查询未来 24h 预测 |
| alert_event | `idx_trigger_time` | 告警时间线 |
| alert_event | `idx_level` | 按级别筛选 |
...
| **密钥管理** | `.env` 文件，已配 `.gitignore` | P0 |
| **SQL 注入防护** | MyBatis-Plus `#{}` 参数化查询 | P0（自动） |
...
| 优先级 | 功能 | 理由 |
|--------|------|------|
| **P0**（Day 4-8，必须） | 数据查询 API + 负荷曲线 + Prophet 预测 + 阈值告警 + 统计卡片 + Agent 基础 NL 查询 | 核心链路，缺一个演示就不完整 |
| **P1**（Day 9-12，尽量） | LSTM 预测 + WebSocket 实时推送 + 大屏自适应 + 告警规则配置页 | 答辩亮点 |
| **P2**（Day 13-15，加分） | JWT 认证 + Agent 多轮对话 + 模型版本管理 + 模型精度监控页面 | 锦上添花 |
| **砍掉** | 告警文案 LLM 生成、3-sigma 检测器、天气数据关联、用户管理 | 时间黑洞，性价比低 |
