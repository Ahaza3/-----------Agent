...
> **配套文档**：[需求规格说明书](./01-需求规格说明书.md) ｜ [技术选型方案](./03-技术选型方案.md) ｜ [接口设计文档](./04-接口设计文档.md) ｜ [数据模型设计](./05-数据模型设计.md) ｜ [部署方案设计](./06-部署方案设计.md)
...
| 原则 | 说明 | 实践方式 |
|:-----|:-----|:---------|
| 🎯 **务实优先** | 16 天实训周期内可交付，不过度设计 | 单模块单体 + 先跑通核心链路再横向扩展 |
| 🔗 **核心链路优先** | Sprint 1 结束前数据 → 展示链路必须贯通 | 模拟数据 → API → 前端曲线图，全链路打通后再加功能 |
| 🔄 **有备选方案** | 每个技术风险点都有 fallback | Flask 微服务主力推理；旧 Prophet 产物仅作历史兼容 |
...
├──────────────────────────────────────────────────────────────┤
│                     ML（训练/推理分离）                           │
│  训练（离线，手动执行）: Python PyTorch LSTM + pandas               │
...
> **为什么不拆 Maven 多模块？** 16 天项目体量小，多模块增加 pom.xml 管理成本和构建复杂度。单模块打包一个 Jar，Dockerfile 也简单。
...
│   ├── train_prophet.py      # Prophet 历史兼容脚本（不再作为重训练入口）
...
| 表名 | 说明 | 核心字段 | 预估行数 |
|:-----|:-----|:---------|:---------|
| `load_data` | 历史负荷数据 | time (UNIQUE), load_mw, temperature, humidity, is_holiday, hour, day_of_week, month | ~17,520 |
| `prediction_result` | 预测结果 | predict_time, predicted_load, lower_bound, upper_bound, model_version_id | ~720 |
| `alert_event` | 告警事件 | trigger_time, level (RED/ORANGE/YELLOW), type, current_value, threshold_value, ai_analysis | ~500 |
| `alert_rule` | 告警规则 | name, type, config (JSON), is_active | ~10 |
| `model_version` | 模型版本 | model_name, version, mape, file_path, is_active | ~5 |
| `user` | 用户 (P2) | username (UNIQUE), password_hash, role | ~5 |
...
> 📄 **完整 DDL、字段注释、数据字典、Flyway 迁移方案** → [05-数据模型设计](./05-数据模型设计.md)
...
```java
// R.java — 统一响应封装 { code, message, data, timestamp }
// code=0 成功，非0 失败（见错误码表）
@Data
public class R<T> {
    private int code;
...
| 场景 | 协议 | 端点示例 |
|:-----|:-----|:---------|
| 数据查询 | REST (HTTP/JSON) | `GET /api/v1/data/load?start=&end=` |
| Agent 对话 | SSE（Server-Sent Events） | `POST /api/v1/agent/chat`（流式 `thinking → text → chart → done`） |
| 实时推送 | WebSocket（STOMP over SockJS） | `/ws/dashboard` → `/topic/load`, `/topic/alerts`, `/topic/predictions` |

> 📄 **完整请求/响应示例、错误码、分页格式、SSE 事件流、STOMP 订阅示例** → [04-接口设计文档](./04-接口设计文档.md)

---

## 6. 开发与部署

### 6.1 开发阶段（Sprint 1-3）：轻量本地环境
...
├── nginx           → :80    反向代理 + 静态资源
├── backend         → :8080  Spring Boot Jar（多阶段构建）
├── frontend        → —     Vite build 产物，Nginx 挂载
├── mysql           → :3306  数据卷持久化 + health check
├── redis           → :6379  缓存 + 会话
└── ml-inference    → :5000  Flask 推理服务
```

**CI/CD**：GitHub Actions，`on: [push, pull_request]`，并行跑 `mvn test` + `npm ci && lint && vitest`。

**多阶段 Docker 构建**：`maven:3.9-eclipse-temurin-17-alpine` 构建 → `eclipse-temurin:17-jre-alpine` 运行（~200MB 最终镜像）。

> 📄 **完整 docker-compose.yml、3 个 Dockerfile、Nginx 配置、CI YAML、健康检查、故障预案** → [06-部署方案设计](./06-部署方案设计.md)

---

## 7. 安全设计（P2，Day 10+ 有余力再做）

> **Sprint 1 阶段所有 API 裸奔**，先把核心功能跑通。演示时没人测你的登录，但所有人都会看大屏和预测曲线。
...
| 优先级 | 时间 | 功能 | 理由 |
|:-------|:-----|:-----|:-----|
| 🔴 **P0** 必须 | Sprint 1-2 | 数据查询 API + 负荷曲线 + LSTM 预测 + 阈值告警 + 统计卡片 + Agent 基础 NL 查询 | 核心链路，缺一个演示就不完整 |
| 🟠 **P1** 尽量 | Sprint 3 | LSTM 预测 + WebSocket 实时推送 + 大屏自适应 + 告警规则配置页 | 答辩亮点，差异化的关键 |
| 🟢 **P2** 加分 | Sprint 3 有余力 | JWT 认证 + Agent 多轮对话 + 模型版本管理 + 模型精度监控页面 | 锦上添花，有余力再做 |
...
### 8.1 Agent SSE 与会话持久化

当前 Agent 对话由 `AgentController → AgentService → AgentCore` 处理。`AgentService` 负责 SSE 流式输出、历史消息加载和会话持久化，`AgentCore` 负责 LLM Function Calling 编排。

- SSE 事件顺序：`thinking → text → chart → done`，异常时输出 `error → done`。
- `text` 事件一次性发送完整 Markdown 文本，避免换行被逐行 trim 后影响 Markdown 渲染。
- `chart` 事件使用 `{ "option": ... }` 包装 ECharts option。
- assistant 消息的图表配置保存到 `conversation.chart_option`，历史会话接口返回 `chartOption`，前端解析后恢复图表。

### 8.2 大屏曲线分段设计

大屏负荷曲线拆为历史段、恢复段、视觉连接段、实时段四类数据：

- `historyData`：真实历史/模拟历史负荷。
- `recoveryData`：后端补齐恢复数据，`data_source = RECOVERED_SIMULATION`。
- `bridgeData`：前端按 15 分钟间隔生成的 smoothstep 插值点，只用于视觉连接恢复段和首个真实实时点。
- `realtimeData`：后端 WebSocket/实时接口提供的真实实时点。

`bridgeData` 不写库、不推送、不导出，不会改变后续整点归档或统计口径。

### 8.3 告警研判与工单策略

告警链路拆为两层：

- **低延迟告警触发层**：`ThresholdDetector` 判级，`AlertTemplate` 生成 `alert_event.ai_analysis` 和 `suggestion`，保证告警延迟目标。
- **工单详情智能研判层**：`AlertJudgementService` 在详情页/重新研判时调用 `LlmClient` 生成 `decisionReason`、`dispatcherAdvice`、`operatorAdvice`；失败时标记 `RULE_BASED_FALLBACK` 并返回规则研判。
- 红色告警只将 `shouldCreateTicket` 置为 true，`autoCreateTicket` 保持 false。前端弹出“待确认工单草稿”，调度员点击“提交工单”后才调用正式建单接口。

### 8.4 工单详情状态隔离

`AlertTicketDetail` 在外部 `ticket` 变化、Drawer 关闭或 `ticketId` 为空时清空 `actions`。未建单告警只展示告警和研判信息，不展示上一次工单的处置时间线。

> 当前架构与代码状态以 [当前实现同步说明](./14-当前实现同步说明.md) 为准。
