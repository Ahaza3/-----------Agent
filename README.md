...
- **通知与天气来源**：调度员可在大屏主动开启浏览器告警通知；模型管理页明确标注当前预测使用历史温度/湿度辅助，暂未接入未来天气预报。
- **未来天气接入第一阶段**：新增 `weather_forecast` 预报快照表、Open-Meteo 定时同步和 `/api/v1/weather/forecast` 查询接口；预测请求会将缓存的未来温度/湿度作为可选 `future_weather` 字段传给 Flask，缓存或服务不可用时自动回退。
